"""
Dictionary aka Associative Arry aka Hash Table is an array based data structure whose indices store pointers to the elements of the dictionary.
...
chaining (hash table implementation in Perl) or open addressing (dictionary implementation in Python).
...
class Entry(object):
    """Entry represents an element of a dictionary containing key, value, hash(key), and next (pointer to the next element at the same index position).
    ...
    def __init__(self, key=None, value=None, hash=None, next=None):
        ...
        self.next  = next  # pointer to next entry at same index position
...
class Dictionary(object):
    ...
    def __str__(self):
        ...
        >>> print(d)
        {'a': 1, 'b': 5, 'o': 5, 'h': 5, 'g': 57, 'v': 49, 'e': 17}
    ...
    def __setitem__(self, key=None, value=None):
        """Python special method which will allow us to add elements to a dictionary. Key and value cannot be None.
        ...
        if self.count > self.size * 2/3:
            # double size of array when array is at 2/3 of occupancy
    ...
    def __delitem__(self, key=None):
        """Python special method which will allow us to remove an element from a dictionary. This can be accomplished in two ways:

        1. Lazy deletion  - where we assign None to the key
        2. Eager deletion - where we remove the key value pair from the dictionary

        This implementation of the method implements eager deletion.
        ...
        """
        if key is None:
            raise ValueError('Must provide key to remove from dictionary.')
    ...
    def _get_index(self, hash=None):
        ...
        if index > self.mask:
            raise IndexError('Index for key outside of dictionary bounds.')

        return index


    def _resize_entries(self):
        """Private helper method which doubles the size of the entries array.

        Return: New size of the entries array.

        O(n) - time complexity
        """
        self.size *= 2 # power of 2 will allow us to use masking instead of modulus to get a fair distribution of indexes
        self.mask  = self.size - 1
        entries = [None] * self.size # new array double the size of old arrray

        # copy entries to new array
        for i, entry in enumerate(self.entries):
            if entry and entry.hash:
                index = self._get_index(entry.hash) # get new index for hash
                entries[index]  = entry # copy entry over to new array, creates additional reference
